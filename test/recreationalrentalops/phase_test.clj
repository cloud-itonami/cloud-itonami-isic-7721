(ns recreationalrentalops.phase-test
  "Unit tests of `recreationalrentalops.phase` rollout logic."
  (:require [clojure.test :refer [deftest is testing]]
            [recreationalrentalops.phase :as phase]))
...
(deftest phase-1-rental-record-only
  (testing "phase 1 allows only rental-record logging, requires approval"
    ...
    (let [{:keys [disposition]} (phase/gate 1 {:op :schedule-fleet-operation} :commit)]
      (is (= :hold disposition)))))

(deftest phase-2-adds-coordination-ops
  (testing "phase 2 allows coordination ops, still requires approval"
    (doseq [op [:log-rental-record :schedule-fleet-operation :coordinate-fleet-restock]]
...
(deftest phase-3-auto-commits-clean-ops
  (testing "phase 3 auto-commits clean, high-conf non-safety ops"
    ...
    (let [{:keys [disposition]} (phase/gate 3 {:op :schedule-fleet-operation} :commit)]
      (is (= :commit disposition)))
    (testing ":coordinate-fleet-restock is NOT among them -- its cost is
              self-declared with no filed catalog to check it against, and an
              unverifiable amount must not buy an auto-commit"
      (let [{:keys [disposition reason]} (phase/gate 3 {:op :coordinate-fleet-restock} :commit)]
        (is (= :escalate disposition))
        (is (= :phase-approval reason))))))

(deftest fleet-restock-never-auto-commits-at-any-phase
  (testing "the structural invariant, agreed by the phase table and the governor"
    (doseq [ph (keys phase/phases)]
      (is (not (contains? (:auto (get phase/phases ph)) :coordinate-fleet-restock))
