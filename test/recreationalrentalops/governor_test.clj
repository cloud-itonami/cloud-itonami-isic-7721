(ns recreationalrentalops.governor-test
  "Pure unit tests of `recreationalrentalops.governor/check` against
...
(def unit-1 {:asset-id "unit-1" :name "Downhill ski set, 170cm, adjustable bindings" :registered? true :verified? true})
(def unit-3 {:asset-id "unit-3" :name "Inflatable kayak, awaiting inspection" :registered? true :verified? false})

(defn- clean-proposal [op asset-id]
  {:op op :asset-id asset-id :summary "s" :rationale "routine recreational equipment rental operations coordination"
...
(deftest effect-not-propose-is-hard
  (testing "any :effect other than :propose is a HARD, un-overridable block"
    (let [s (store/mem-store {"unit-1" unit-1})
          verdict (gov/check {} nil (assoc (clean-proposal :schedule-fleet-operation "unit-1") :effect :commit) s)]
      (is (true? (:hard? verdict)))
...
(deftest finalize-safety-clearance-content-is-hard
  (testing "a proposal that claims to directly finalize the safety clearance is HARD-blocked, same as re-rent-without-inspection"
    (let [s (store/mem-store {"unit-1" unit-1})
          poisoned (assoc (clean-proposal :schedule-fleet-operation "unit-1")
  ...
  (testing "flagging observed defect/damage/malfunction concerns (including a mention of a failed post-return inspection as raw observation, not a finalized clearance decision) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"unit-1" unit-1})
          concern (assoc (clean-proposal :flag-equipment-safety-concern "unit-1")
                         :value {:concern "binding release force out of spec on unit-7; unit failed the post-return safety inspection"})
