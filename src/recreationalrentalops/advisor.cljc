(ns recreationalrentalops.advisor
  "RecreationalRentalOpsAdvisor -- the *contained intelligence node* for
  the ISIC-7721 recreational and sports goods rental
  operations-coordination actor (skis, bicycles, watersports and
  camping equipment rental -- residual to 7710 motor vehicles / 7722
  video tapes and disks / 7729 other personal and household goods).

  It drafts exactly four kinds of back-office proposal from a closed
  allowlist: rental-record logging (checkout / return / inspection-note
  data), equipment-availability/maintenance scheduling, rental-fleet
  ...
  *proposal* (with a rationale + the fields it cited), never a
  ...
  by `recreationalrentalops.governor` before anything touches the SSoT.

  This advisor NEVER drafts a direct finalization of an
  equipment-safety-clearance decision (e.g. certifying a returned ski
  set / bike / kayak safe to re-rent without inspection) or an override
  of an equipment-safety-authority decision -- those are permanently
  out of scope for this actor, not merely un-implemented (Wave 4
  person-facing-service safety guardrail, ADR-2607152500).
  `recreationalrentalops.governor`'s `scope-exclusion-violations`
  ...
  `recreationalrentalops.governor/scope-excluded-terms` scans for. See
  that namespace's docstring and
...
(defn- propose-fleet-operation
  "Draft an equipment-availability/maintenance scheduling PROPOSAL only
  (block out a unit for upcoming availability, or schedule a
  maintenance/tune-up pass -- never a direct re-rent-clearance
  decision)."
  [_db {:keys [asset-id patch]}]
  {:op         :schedule-fleet-operation
   :asset-id   asset-id
   :summary    (str asset-id " のフリート運用（空き状況/メンテナンス）予定提案: " (pr-str (keys patch)))
   :rationale  "空き状況調整またはメンテナンス日程調整の提案のみ。再貸出可否の最終判断は人間の点検担当者が行う。"
   :cites      [asset-id]
   :effect     :propose
   :value      (merge {:asset-id asset-id} patch)
   :confidence 0.89})
...
(defn- propose-equipment-safety-concern
  "Surface an equipment safety concern (defect, damage, malfunction --
  e.g. a ski-binding release-force fault, a bicycle brake defect, a
  kayak hull puncture) for HUMAN triage. This op ALWAYS escalates in
  `recreationalrentalops.governor` -- never auto-committed at any
  phase -- regardless of how confident the advisor is that the concern
  is real, and it never itself finalizes an equipment-safety-clearance
  decision or overrides an equipment-safety-authority decision."
  [_db {:keys [asset-id patch]}]
