(ns recreationalrentalops.governor
  "RecreationalRentalGovernor -- the independent compliance layer that
  earns the RecreationalRentalOpsAdvisor the right to commit. The
  advisor has no notion of whether a rental-asset record is actually
  ...
  (rental-record logging, equipment-availability/maintenance
  scheduling, rental-fleet restock/replacement coordination,
  equipment-safety-concern flagging). It NEVER performs or authorizes:
    - directly finalizing an equipment-safety-clearance decision (e.g.
      certifying a returned ski set / bike / kayak safe to re-rent
      without inspection)
  ...
  (ADR-2607152500): recreational/sports-equipment rental (skis,
  bicycles, watersports and camping gear) has a direct end-user-safety
  dimension -- defective or uninspected gear (a ski-binding
  release-force fault, a bicycle brake defect, a kayak hull puncture)
  causing injury -- so the closed op allowlist NEVER includes any op
  that directly finalizes an equipment-safety-clearance decision;
  those are always either a hard permanent block or an always-escalate
  op, never auto-commit-eligible. `:flag-equipment-safety-concern`
  ...
  `recreationalrentalops.phase` independently agrees:
...
(def high-cost-threshold
  ...
  human, regardless of confidence -- routine consumable/accessory
  restocks (helmets, paddles, tune-up wax) sit well under this, so
  this only catches unusually large procurement/replacement orders
  (e.g. a bulk ski-set or e-bike-fleet replenishment)."
  2000)
...
(def scope-excluded-terms
  ...
  equipment-safety-clearance decision (e.g. certifying a returned ski
  set / bike / kayak safe to re-rent without inspection) or overriding
  an equipment-safety-authority decision. Scanned across the
  ...
  (see `recreationalrentalops.governor-test`'s own
  `legitimate-equipment-safety-concern-is-not-scope-excluded` and
  `recreationalrentalops.advisor-test`'s
...
(defn- high-cost-fleet-restock?
  ...
  `recreationalrentalops.phase` independently agrees -- `:coordinate-fleet-restock` is no
...
(defn check
  "Censors a RecreationalRentalOpsAdvisor proposal against the governor
