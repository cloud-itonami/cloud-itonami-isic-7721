(ns recreationalrentalops.operation
  ...
  human operator/fleet manager. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [recreationalrentalops.advisor :as advisor]
            [recreationalrentalops.governor :as governor]
...
(defn build
  ...
  `recreationalrentalops.store/Store`).
  opts:
    :advisor      -- a `recreationalrentalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RecreationalRentalOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; RecreationalRentalGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :asset-id (:asset-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :asset-id (:asset-id request) :by (:by approval)}]}
            {:disposition :hold
