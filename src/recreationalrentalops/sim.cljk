(ns recreationalrentalops.sim
  ...
  auto-commit), then an equipment-availability/maintenance scheduling
  request and a fleet-restock coordination request (both auto-commit
  clean at phase 3), then an equipment-safety-concern flag (ALWAYS
  escalates, at any phase -- approve, then commit), then HARD-hold
  scenarios: an unregistered unit, a unit registered but not yet
...
(defn -main [& _]
  (let [db (store/seed-db)
    ...
    (let [r (exec-op actor "t1" {:op :log-rental-record :asset-id "unit-1"
                                  :patch {:renter "Alex Kim" :checkout "2026-07-14" :days 3}} manager-phase-1)]
      (println r)
    ...
    (println (exec-op actor "t2" {:op :log-rental-record :asset-id "unit-1"
                                  :patch {:renter "Alex Kim" :return "2026-07-17"}} manager-phase-3))

    (println "\n== schedule-fleet-operation unit-1 (phase 3, clean -- auto-commits) ==")
    (println (exec-op actor "t3" {:op :schedule-fleet-operation :asset-id "unit-1"
                                  :patch {:item "binding release-force calibration" :urgency "routine"}} manager-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-fleet-restock :asset-id "unit-1"
                                  :patch {:item "replacement ski poles" :quantity 6 :estimated-cost 300}} manager-phase-3))
    ...
    (let [r (exec-op actor "t4b" {:op :coordinate-fleet-restock :asset-id "unit-1"
                                  :patch {:item "replacement ski-set fleet order" :quantity 10 :estimated-cost 15000}} manager-phase-3)]
      (println r)
      (println "-- human fleet manager approves --")
    ...
    (let [r (exec-op actor "t5" {:op :flag-equipment-safety-concern :asset-id "unit-1"
                                 :patch {:concern "binding release force out of spec after last return" :confidence 0.92}} manager-phase-3)]
      (println r)
    ...
    (println "\n== schedule-fleet-operation unit-1, advisor attempts direct actuation (:effect :commit) -> HARD hold ==")
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
                                                (-advise [_ _ req]
                                                  (assoc (advisor/infer nil req) :effect :commit)))})]
      (println (exec-op actor-direct "t8" {:op :schedule-fleet-operation :asset-id "unit-1"
